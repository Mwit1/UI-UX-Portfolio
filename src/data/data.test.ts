...
describe('Gallery images', () => {
    test('Gallery images handles various file types', async () => {
        await getGalleryImages([100, 512, 1024, 1800, 3600], true, 'Restaurant UI').then(({props}) => {
            expect(props).toBeDefined()
            expect(props.list).toBeDefined()
            expect(props.list.length).toBe(5)
            expect(props.list[0].srcSet?.length).toBe(6)
            expect(props.list[0].src).toBe('/gallery/01-UI/01-UIUX.jpg')
            expect((props.list[0] as any).description).toBe('Restaurant UI')
            expect(props.list[0].srcSet).toBeDefined()
            expect(props.list[0].srcSet![0].src).toBe('/gallery/01-UI/01-UIUX.jpg')
        })
    }, 60000)  // stupidly long timeout, but GitHub CI isn't exactly overpowered

    test('Gallery images creates srcSet', async () => {
        await getGalleryImages([100, 512, 1024, 1800, 3600], true, '01-UI').then(({props}) => {
            expect(props).toBeDefined()
            expect(props.list![0].srcSet).toBeDefined()
            expect(props.list![0].srcSet!.length).toBe(6)
            expect(props.list![0].src).toBe('/gallery/01-UI/01-UIUX.jpg')
...
describe('Gallery Lists', () => {
    ...
    test('Gallery list creates gallery without _index.mdx', () => {
        ...
        expect(galleries).toBeDefined()
        expect(galleries.length).toBe(3)
        expect(galleries[2].key).toBe('More Designs')
    })
    ...
    test('Gallery metadata parsed from _index.mdx', () => {
        ...
        expect(galleries[1].caption).toBe('These are THE deSIGNS - follow them')
    })
